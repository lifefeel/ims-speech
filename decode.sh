...
steps/make_fbank_pitch.sh --cmd run.pl --nj ${nj} --write_utt2num_frames true ${workdir}/trans/${recid} ${workdir}/feats/${recid}
utils/fix_data_dir.sh ${workdir}/trans/${recid}
dumpdir=${workdir}/dump/${recid}
dump.sh --cmd run.pl --nj ${nj} --do_delta false ${workdir}/trans/${recid}/feats.scp ${model}/cmvn.ark ${workdir}/exp/${recid} ${dumpdir}
...
run.pl JOB=1:${nj} ${dumpdir}/log/decode.JOB.log \
	asr_recog.py \
		--ngpu 0 \
		--backend pytorch \
...
sed -i.bak2 -r 's/<blank> //g' ${dumpdir}/hyp.trn
